...
function you_win(){
    winscreen = game.add.text(160, 250, "Congratulations, you won", {fontSize: '50px', fill: '#00FF00'}); 
    winscreen_2 = game.add.text(210,650, "ctrl + r to restart i guess", {fontSize: '50px', fill: '#00FF00'});
    ...
    if (send_only_2 == 1){
        setTimeout(LeaderBoard(),5000)
    }
}  

function you_lose(){
    winscreen = game.add.text(160, 250, "you lose", {fontSize: '50px', fill: '#00FF00'}); 
    winscreen_2 = game.add.text(210,650, "ctrl + r to restart i guess", {fontSize: '50px', fill: '#00FF00'});
    setTimeout(KillAllSprites(),1000)
    ...
    if (continue_onwards == 1){
        if (send_only_1 == 1)
        {
            ...
            MongoClient.connect(url, function(err, db) {
                assert.equal(null, err);
                console.log("Connected correctly to server.");
                var myobj = {score:score,highname:ScoreName}
                db.collection("scoreboard_new").insertOne(myobj, function(err, res){
                if (err) throw err;{
                    console.log("all good");
                    db.close();
                    var scoreText1 = game.add.text(16,0,"H I G H - S C O R E S ", {fontSize: '32px', fill: '#FFFFFF'});
                    var spliced = result
                    score1 = result.slice(0,1)
                    score1 = JSON.stringify(result[0]['score'])
                    console.log(score1)
                    var scoreText3 = game.add.text(16,45,score1, {fontSize: '32px', fill: '#FFFFFF'});
                    score2 = result.slice(1,2)
                    score2 = JSON.stringify(result[1]['score'])
                    console.log(score2)
                    var scoreText5 = game.add.text(16,90,score2, {fontSize: '32px', fill: '#FFFFFF'});
                    score3 = result.slice(2,3)
                    score3 = JSON.stringify(result[2]['score'])
                    console.log(score3)
                    var scoreText7 = game.add.text(16,135,score3, {fontSize: '32px', fill: '#FFFFFF'});               
                    score4 = result.slice(3,4)
                    score4 = JSON.stringify(result[3]['score'])
                    console.log(score4)
                    var scoreText9 = game.add.text(16,180,score4, {fontSize: '32px', fill: '#FFFFFF'});
                    score5 = result.slice(4,5)
                    score5 = JSON.stringify(result[4]['score'])
                    console.log(score5)
                    var scoreText11 = game.add.text(16,225,score5, {fontSize: '32px', fill: '#FFFFFF'});
                //Splicing up the array into the parts needed
                    var scoreText2 = game.add.text(350,0,"N A M E S", {fontSize: '32px', fill: '#FFFFFF'});
                    name1 = result.slice(name1)
                    name1 = JSON.stringify(result[0]['highname'])
                    console.log(name1)
                    var scoreText4 = game.add.text(350,45,name1, {fontSize: '32px', fill: '#FFFFFF'});
                    name2= result.slice(name2)
                    name2 = JSON.stringify(result[1]['highname'])
                    console.log(name2)
                    var scoreText6 = game.add.text(350,90,name2, {fontSize: '32px', fill: '#FFFFFF'});
                    name3 = result.slice(name3)
                    name3 = JSON.stringify(result[2]['highname'])
                    console.log(name3)
                    var scoreText8 = game.add.text(350,135,name3, {fontSize: '32px', fill: '#FFFFFF'});               
                    name4 = result.slice(name4)
                    name4 = JSON.stringify(result[3]['highname'])
                    console.log(name4)
                    var scoreText10 = game.add.text(350,180,name4, {fontSize: '32px', fill: '#FFFFFF'});
                    name5 = result.slice(name5)
                    name5 = JSON.stringify(result[4]['highname'])
                    console.log(name5)
                    var scoreText12 = game.add.text(350,225,name5, {fontSize: '32px', fill: '#FFFFFF'});
                }})})
        }
    }
    if (send_only_2 == 1){
...
function LeaderBoard(){
    ...
    // if ((server_ready == 1) && (server_ready2 == 3)){
    //     display_scoreboard()
    // }
    ...
    if (send_only_2 == 1){
        ...
        console.log("Connected correctly to server.");
            var sort = { "score" : -1 };
            var grabbed_high_scores = db.collection("scoreboard_new").find().sort(sort).limit(5).toArray(function(err, result) {
            if (err) throw err;{
...
function wasting_time(){
    setTimeout(display_scoreboard,1000)
}
